...
为 Finch 对话添加**规划模式**按钮。开启后，AI 助手只会输出方案，不会执行任何操作，每轮回复结束后会询问你是否开始执行。

## 使用方法

1. 点击对话小工具栏中的**规划模式**按钮（📋）开启。
2. 像往常一样发送任务，助手只会输出结构化方案。
3. 每轮回复结束后，Composer 底部出现确认条：
   - **执行** — 关闭规划模式，自动发送一条引导语驱动助手执行方案。
   - **继续规划** — 保持规划模式，继续下一轮规划。
4. 随时再次点击按钮手动退出规划模式。

**首页：** 在首页开启规划模式，仅对下一条消息生效，发送后自动进入新对话并保持规划模式。
