...
## 6. Protocol version & standard request headers

MCP Client is built on the official `@modelcontextprotocol/client` 2.0 SDK. It enables
`versionNegotiation: { mode: 'auto' }`, which probes the server for the modern protocol era
(`2026-07-28` and later) during `initialize` and falls back to the plain legacy handshake for
older servers. **SDK major version ≠ protocol version**: never assume "SDK 2.0" means a
specific protocol date — always reason from the version actually negotiated in the
`initialize` response.

For Streamable HTTP servers negotiated at `2026-07-28`, the official transport mirrors JSON-RPC
body fields into HTTP headers for intermediaries to route and inspect:

| Header | Source field | Required for |
|---|---|---|
| `Mcp-Method` | `method` | All requests |
| `Mcp-Name` | `params.name` or `params.uri` | `tools/call`, `resources/read`, `prompts/get` |
| `MCP-Protocol-Version` | negotiated version in `_meta` | Every POST |

These headers are **dynamically generated by the official transport per request** — their values
must match the request body (a `tools/call` request carries `Mcp-Name: <params.name>`, a
`resources/read` carries `Mcp-Name: <params.uri>`). Servers validate header/body consistency and
reject mismatches with a `HeaderMismatch` error (`-32020`, HTTP 400), for example
"the body carries params.name=\"...\" but the required Mcp-Name header is absent".

**Never configure protocol headers manually.** The `headers` field in `registerServer()` /
`servers.json` is only for business headers such as `Authorization`, `X-Api-Key`, `X-Tenant-Id`.
Do not set `MCP-Protocol-Version`, `Mcp-Method`, `Mcp-Name`, or `Mcp-Param-*` there — a fixed
`Mcp-Name` cannot match the per-request body and is not a valid workaround. If a server rejects
with a header-validation error, investigate the client/transport layer (negotiated version,
SDK lockfile, any wrapper around the official transport) rather than the mini tool manifest.
...
## 8. Why not write to servers.json?
...
When the connection succeeds but tool calls fail (or a `HeaderMismatch` / protocol-version error
appears), go deeper — a healthy connection state only proves `initialize`/tool discovery, not
that `tools/call` works. Trace in this order:

1. Confirm the **negotiated protocol version** from the `initialize` response
2. Identify the failing request type (`tools/call`, `resources/read`, `prompts/get`, …)
3. Compare `method`, `params.name` / `params.uri` in the request body with the headers actually sent
4. Check the MCP SDK version and its lockfile — SDK 2.0 negotiates `2026-07-28`; older SDKs send legacy versions
5. Check whether the official transport's request construction is wrapped, bypassed, or overridden
6. Only last, check the mini tool's business custom headers in the manifest — protocol headers are
   never the mini tool's responsibility (see §6)
