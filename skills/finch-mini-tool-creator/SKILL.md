...
- `ctx.events` — read-only Agent runtime event subscription (`onAgentEvent`), delivered best-effort; listener errors never break the Agent flow. By default events contain sanitized status metadata. A mini tool that must observe complete events may declare `permissions.agentEvents: "full"`; after permission grant it receives content fields including prompts, assistant text/thinking, tool inputs/results, file paths, and attachment metadata/content across **all** Agent Sessions, plus `runId` and the effective `cwd`. Treat this as highly sensitive, never log or persist it by default, and do not use it when sanitized telemetry is sufficient.
